...
public class Select2 extends Debuggable{
	...
	*/
	...
	public boolean execute(Closure closure){
		...
		if (g_debug){ g_step(Step.MARK_ACTIVE); }
		active[i] = true;
		if (g_debug){ g_step(Step.MARK_ACTIVE_DONE); }
		
		// 2. check whether I am the token owner
		if (g_debug){ g_step(Step.IS_OWNER); }
		boolean token_owner = token == i;
		if (g_debug){ g_step(Step.IS_OWNER_DONE); }
		...
		if (active[(i + 1) % 2]){
			...
			if (!token_owner){
				if (g_debug){ g_step(Step.OWNER_ACTIVE); }
				wait[(i+1) % 2] = false;
				active[i] = false;
				if (g_debug){ g_step(Step.OWNER_ACTIVE_DONE); g_finish_round(); }
				return false;
			}
			// 3.2. if I am the token owner wait for the other thread till it decides what to do 
				...
				wait[i] = true;
				while ( true ){
					if (g_debug){ g_step(Step.WAIT_CONDITION); }
					if (token != i || !active[(i + 1) % 2] || !wait[i]){ break; }
					if (g_debug){ g_step(Step.WAIT_CONDITION_DONE); }
					Thread.yield();
				}
				wait[i] = false;
			}
		...
		if (token_owner){
			...
			if (token != i){
				if (g_debug){ g_step(Step.LOST_OWNERSHIP); }
				active[i] = false;
				if (g_debug){ g_step(Step.LOST_OWNERSHIP_DONE); g_finish_round(); }
				return false;
			}
			// 4.2. if I was and still is the token owner, then I am selected, give up the token ownership, cleanup and exit
			else{
				if (g_debug){ g_step(Step.OWNER_SELECTED); }
				selected[i] = true;
		...
		// 4.3. if I was not the token owner but reached this point, than I am selected, get the token ownership, cleanup and exit
			...
			selected[i] = false;
			active[i] = false;
			if (g_debug){ g_step(Step.NOT_OWNER_SELECTED_DONE); g_finish_round(); }
			return result;
		}
	}
